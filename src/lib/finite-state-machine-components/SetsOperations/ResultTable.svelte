<!--
    ResultTable.svelte
    This component is responsible for displaying the table for the result automaton.
    Author: Marek Krúpa
-->
...
<script lang="ts">
    ...
    $: if ($result_graph_store.generated == true || $result_graph_store.currentStatus) {
        ...
        nodesMeta = $result_graph_store.nodes;


        let alphabet = $result_graph_store.input_alphabet;
        if (alphabet.includes('ε')) {
            alphabet.splice(alphabet.indexOf('ε'), 1);
            alphabet.push('ε');
        }

        if (typeof $result_graph_store.input_alphabet !== 'undefined'){
            cols = [...alphabet];
        }
        ...
        if (typeof transitions !== 'undefined' && generated === false) {
            ...
            nodes.forEach(node => {
                ...
                let node_id = $result_graph_store.nodes.filter(n => n.label === node)[0].id;

                if ($result_graph_store.startState.includes(node_id)) {
                    rowData.node = '-> ' +  rowData.node;
                }

                if ($result_graph_store.finishState.includes(node_id)) {
                    rowData.node = '<- ' + rowData.node;
                }
        ...
        $result_graph_store.generated = false;
    }
    ...
    <div class="table">
        ...
        <div class="tableBody">
            {#each tableData as row}
                {#if $result_graph_store.currentStatus !== undefined &&
                $result_graph_store.traversal[$result_graph_store.currentStep + 1 ] !== undefined}
                    {#if (row.node == '-> ' + $result_graph_store.traversal[$result_graph_store.currentStep +1 ].stateLabel ||
                        row.node == '<- ' + $result_graph_store.traversal[$result_graph_store.currentStep +1 ].stateLabel ||
                        row.node == $result_graph_store.traversal[$result_graph_store.currentStep +1 ].stateLabel)}
